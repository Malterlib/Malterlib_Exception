...
#define DMibExceptionTraceEnable
		...
		public:
			...
			CExceptionBase(const ch8 *_pClass, const ch8 *_pFile, aint _Line, const ch8 *_pFunction, const ch8 *_pError, bool _bTrace, bool _bStackTrace);
			CExceptionBase(const ch8 *_pClass, const ch8 *_pFile, aint _Line, const ch8 *_pFunction, NStr::CStr const &_Error, bool _bTrace, bool _bStackTrace);
			CExceptionBase(const ch8 *_pClass, const ch8 *_pFile, aint _Line, const ch8 *_pFunction, NStr::CStrNonTracked const &_Error, bool _bTrace, bool _bStackTrace);

			template <typename t_CError>
			CExceptionBase(const ch8 *_pClass, const ch8 *_pFile, aint _Line, const ch8 *_pFunction, NStr::TCStrAggregate<t_CError> const &_Error, bool _bTrace, bool _bStackTrace);
			template <typename t_CError>
			CExceptionBase(const ch8 *_pClass, const ch8 *_pFile, aint _Line, const ch8 *_pFunction, NStr::TCStr<t_CError> const &_Error, bool _bTrace, bool _bStackTrace);
		...
		public:
			template <typename t_CError>
			TCException(const ch8 *_pClass, const ch8 *_pFile, aint _Line, const ch8 *_pFunction, t_CError &&_Error, bool _bTrace, bool _bStackTrace = true)
				: CExceptionBase(_pClass ? _pClass : fg_GetTypeName<TCException>(), _pFile, _Line, _pFunction, fg_Forward<t_CError>(_Error), _bTrace, _bStackTrace)
			{
			}
			template <typename t_CError>
			TCException(const ch8 *_pClass, t_CError &&_Error, bool _bTrace, bool _bStackTrace = true)
				: CExceptionBase(_pClass ? _pClass : fg_GetTypeName<TCException>(), "", 0, "", fg_Forward<t_CError>(_Error), _bTrace, _bStackTrace)
			{
			}
		};
...
#		define DMibImpErrorSpecific(d_CClass, d_Description, d_SpecificData) throw d_CClass(nullptr, DMibPFile, DMibPLine, DMibPFunction, d_Description, d_SpecificData, true)
#		define DMibImpExceptionInstanceSpecific(d_CClass, d_Description, d_SpecificData) d_CClass(nullptr, DMibPFile, DMibPLine, DMibPFunction, d_Description, d_SpecificData, false)
...
#		define DMibImpErrorClass(d_CClass, d_CParent) \
			...
			public:\
				template <typename t_CError>\
				d_CClass(const ch8 *_pClass, const ch8 *_pFile, aint _Line, const ch8 *_pFunction, t_CError &&_Error, bool _bTrace)\
					: d_CParent(_pClass ? _pClass : DMibStringize(d_CClass), _pFile, _Line, _pFunction, fg_Forward<t_CError>(_Error), _bTrace)\
				{\
				}\
			}
...
#		define DMibImpErrorSpecificClass(d_CClass, d_CParent, d_CSpecificType) \
			...
			public:\
				template <typename t_CError>\
				d_CClass(const ch8 *_pClass, const ch8 *_pFile, aint _Line, const ch8 *_pFunction, t_CError &&_Error, d_CSpecificType const &_SpecificData, bool _bTrace)\
					: d_CParent(_pClass ? _pClass : DMibStringize(d_CClass), _pFile, _Line, _pFunction, fg_Forward<t_CError>(_Error), _bTrace)\
		...
		public:
			template <typename t_CError>
			CExceptionMemory(const ch8 *_pClass, const ch8 *_pFile, aint _Line, const ch8 *_pFunction, t_CError &&_Error, bool _bTrace)
				: CException(_pClass ? _pClass : "CExceptionMemory", _pFile, _Line, _pFunction, fg_Forward<t_CError>(_Error), _bTrace, false)
			{
			}
		};
