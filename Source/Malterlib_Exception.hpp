...
namespace NMib
{
	namespace NException
	{
		...
		CExceptionBase::CExceptionBase(const ch8 *_pClass, const ch8 *_pFile, aint _Line, const ch8 *_pFunction, NStr::TCStrAggregate<t_CError> const &_Error, bool _bTrace, bool _bStackTrace)
			: m_Magic(mcp_Magic)
		...
		template <typename t_CError>
		CExceptionBase::CExceptionBase(const ch8 *_pClass, const ch8 *_pFile, aint _Line, const ch8 *_pFunction, NStr::TCStr<t_CError> const &_Error, bool _bTrace, bool _bStackTrace)
			: m_Magic(mcp_Magic)
			, m_pClass(_pClass ? _pClass : "CExceptionBase")
			, m_pFile(_pFile ? _pFile : "Unknown")
			, m_pFunction(_pFunction ? _pFunction : "Unknown")
